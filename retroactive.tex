...
For the subgoal trie, we extended each subgoal trie node with a new field called \textbf{in\_eval}
...
subgoal trie nodes in the subgoal trie path get the \textbf{in\_eval} field incremented.
...
the \textbf{in\_eval} field can be equal to either: 1, when the corresponding subgoal is in
...
get the \textbf{in\_eval} field incremented (Figure~\ref{fig:in_eval_add}). Note how the
...
\begin{figure}[ht]
  ...
  \caption{Example subgoal trie extended with the \textbf{in\_eval} field.}
...
chains each trie node where the \textbf{in\_eval} field is greater than 0. Note that
this linked list is not ordered by the \textbf{in\_eval} value. Each evaluation index
node contains the following fields: \textbf{prev}, a pointer to the previous evaluation
index node, if any; \textbf{next}, a pointer to the next evaluation index node, if any;
\textbf{node}, points to the subgoal trie node this index node represents; and
\textbf{in\_eval}, the number of evaluating subgoals under the corresponding subgoal trie node.
We also extended the hash table with a field named \textbf{index} to point to the evaluation
index.

Figure~\ref{fig:hash_table_evaluation_index} shows an hash table and the corresponding
evaluation index. Note that the trie nodes on the index have the \textbf{in\_eval} field
made to point to the index node, while the only trie node with $in\_eval = 0$ is not indexed
and thus does not have an index node. Using the \textbf{status} field of each subgoal trie node
...
from the leaf to the root, and increments the \textbf{in\_eval} field. When we find
hashed trie nodes, we must check if the field is currently being indexed. If this is the case,
we simply increment the \textbf{in\_eval} field of the index node, otherwise we create
a new index node on the evaluation index pointing to the current node and the \textbf{in\_eval}
...
\textbf{in\_eval} field will reach 0, the trie node no longer needs to be indexed,
hence we must remove the index node from the evaluation index. In the other cases,
we simply decrement the respective \textbf{in\_eval} field.
...
\textbf{in\_eval} field described previously we can also prune irrelevant
...
\begin{enumerate}
  ...
  \item Search for the next node with a valid \textbf{in\_eval} field to be pushed on the choice point stack,
         if any;
...
terms can have at most one alternative in a trie level. If a node satisfies the \textbf{in\_eval} requisite,
...
node with a symbol that matches our term and has a valid \textbf{in\_eval} field.
...
\begin{enumerate}
   ...
   \texttt{next\_valid\_index\_node} that uses the \textbf{next} pointer of the first
