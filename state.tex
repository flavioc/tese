...
the Yap and XSB Prolog systems, concentrating on the tabling engines designed for both systems.
...
The goal \texttt{edge(X,Y)} is not tabled and is resolved as usual. We use the first \texttt{edge/2} clause with \texttt{X = a, Z = b}
...
execution is made, propagating the binding $Z = b$ to \texttt{path(b,Z)} (step 12) and the first solution to this subgoal is found and stored.
...
to subgoal \texttt{path(X,Z)}, \texttt{X = a, Z = a}.
...
Execution then proceeds by executing each predicate clause. This is performed by
\texttt{table\_retry}, which alters the next instruction of the generator choice point to the next clause
of the predicate.
On the other hand, if the subgoal is repeated,
a consumer choice point is allocated and set to execute the \texttt{answer\_resolution}
instruction, which consumes answers and implements the answer resolution operation.
...
thus by dereferencing them we obtain a new solution, which can be added unto the table space.    
...
method will be greatly reduced. This new approach will be throughly explored in the next chapter.
  ...
  Well-Founded Semantics (tabling with negation) \cite{Gelder-91} through the use of a delaying-based tabling engine, the SLG-WAM \cite{Chen-96}.
