...
\textit{external generator}. This can happen when an \textit{external consumer}
...
make the operation of removing a frame from the stack more efficient.
...
we distinguish the new retroactive evaluation method by calling \texttt{retroactive\_subgoal\_search}.
...
we simply load all the available answers and then call the \texttt{table\_try\_answer}, which will
load all the available answers and then execute the program clauses.
