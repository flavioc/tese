...
point to the start of the chain, but if the trie level is a hash table,
...
number, which points to a heap term. Given that we are trying to match a constant symbol,
...
nodes through a hash table to provide direct node access and therefore optimize the search. Given
that a hash table can have a large number of answer nodes with different time stamp values, we maintain
...
both cases. Figure~\ref{fig:hash_table_tst} illustrates a hash table with a time stamp index.

\begin{figure}[ht]
  \centering
    \includegraphics[scale=0.6]{hash_table_tst.pdf}
  \caption{Indexing nodes through a hash table with time stamp indexes.}
...
(2) a parent node pointing to a sibling chain list; (3) a parent node pointing to a hashed child node.
...
time stamp indexing (see Figure~\ref{fig:tst_chain_insert}), inserting a new node into a hash table does,
...
If the current node is a hashed node with a time stamp index, the corresponding index node is moved to the head of
the index's double linked list. We can test whether an answer node is indexed by
a hash table by inspecting its \texttt{status} field.
...
Creating the index for a hash table amounts to iterating over the hashed nodes
...
term and has a valid time stamp. Node $N$ can lead either to a simple node chain or a hash table.
...
\texttt{unify\_constant\_term}. First, we check for a hash table and inspect the match
  ...
  while (chain != NULL)
    ...
    if (is_variable(symbol)) // case (1)
      ...
      if ((is_functor(symbol(chain)) or is_list(symbol(chain))) and symbol == term)
        ...
        // trie variable bound to a heap structured term
        push_choice_point_frame(choice_point_stack, alt_chain)
        push(term_log_stack, term)
        return chain
    else
...
\begin{enumerate}
  \item The current node is a hash table. In this situation we can select the next transitions
   by using the time stamp index, which can efficiently prune based on the time stamp. Notice that we visit
   a hash table only once, subsequent backtracking uses the time stamp index nodes;
  \item The current node is inside a hash table and thus indexed on the time stamp index.
...
unify_variable_term(variable, node, ts) {
  if (is_hash_table(node))
     // case 1: current node is a hash table
    ...
    // case 2: current node is a hashed node and has a corresponding index node
...
\begin{itemize}
  ...
  \item Variable: if the variable is a trie variable, we bind and trail it; if it is a heap variable that
...
unify_with_variable(variable, symbol, node) {
  if (is_constant(symbol))
    ...
    if (is_list(symbol(node)) or is_functor(symbol(node))
      ...
      // trie variable bound to a heap structure
      bind_and_conditionally_trail(variable, symbol)
...
Two new mutually exclusive flags were created: \textbf{variant},
...
a hash table, a leaf node or a hashed node.
...
Each subgoal frame was also extended with a \texttt{type} field, which identifies the subgoal frame type.
...
The instruction \texttt{trie\_do\_hash} is executed once a hash table node is reached.
...
We use a hash choice point (\texttt{hash\_choice\_pt}) to execute each bucket.
